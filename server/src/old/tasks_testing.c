...
static int search_token(const char *haystack, const char *search_key, char result[32]) {
...
static int search_value(const char *haystack, const char *search_key, char result[215]) {
    ...
    while (!isspace((unsigned char)*value_end) && *value_end != '"' && *value_end != '\0') {
        value_end++;
    }
...
struct HTTP_RESPONSE t_page_delete(char token[33], int page_id) {
...
struct HTTP_RESPONSE t_login(char buffer[1024]) {
    ...
    char email[215];
    char password[215];
    if (search_value(body, "email:", email) != 0 || search_value(body, "password:", password) != 0) {
        strcpy(response.code, "404 Bad Request\r\n");
...
//   return response;
// }
struct HTTP_RESPONSE t_users_user_vaults_vault_pages_page_patch(int user_id, int vault_id, int page_id, char *new_title,
                                                                char *new_content) {
...
struct HTTP_RESPONSE t_users_user_vaults_vault_pages_page_delete(int user_id, int page_id) {
...
struct HTTP_RESPONSE t_users_user_vaults_vault_get(int user_id, int vault_id) {
    ...
    strcpy(response.code, "200 OK\r\n");
    snprintf(response.body, sizeof(response.body), "vault_name = \"%s\"\nvault_page_count = %d", vault_name, vault_page_count);
    return response;
}
struct HTTP_RESPONSE t_users_user_vaults_vault_delete(int user_id, int vault_id) {
...
struct HTTP_RESPONSE t_users_user_vaults_vault_pages_get(int user_id, int vault_id) {
...
struct HTTP_RESPONSE t_users_user_vaults_vault_pages_post(int user_id, int vault_id, char *file_path, char *title,
                                                          char *content) {
...
struct HTTP_RESPONSE t_users_user_vaults_vault_pages_page_get(int user_id, int vault_id, int page_id) {
