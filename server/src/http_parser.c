...
int parse_curly_bracket_contents(const char *source, char *key, char **target) {
  ...
  size_t length = close_bracket - open_bracket;
  printf("bad %lu\n", length);
  *target = (char *)malloc(length + 1);

  strncpy(*target, open_bracket, length);

  // *target[(int)length - 4] = '\0';
  printf("OK %lu", strlen(*target));
  return 0;
}
...
int parse_extract_uri_child(const char *source, char *target) {
  ...
  if (strlen(second_slash) < 1 || (strlen(second_slash) > MAX_URI_CHILD_SIZE)) {
